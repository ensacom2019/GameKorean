...
1. Windows 배포본의 `GameKO.exe`를 실행합니다.
2. 번역할 게임 폴더 또는 게임 실행 파일을 선택합니다.
3. 원문 언어와 번역 방식을 확인합니다.
4. Unity라면 기본 XUnity 방식 또는 `Unity 정적 패치(테스트)`를 선택합니다.
5. `자동 번역 시작`을 누릅니다.
6. 작업 완료 후 게임을 실행해 결과를 확인합니다.

게임 폴더나 실행 파일을 `GameKO.exe` 위로 끌어놓아도 됩니다. 배포 폴더를 게임 폴더 안에 두면 부모 게임 폴더도 자동 검사합니다.
...
`원본 복원`은 GameKO가 변경하거나 추가한 파일만 대상으로 합니다. 번역 적용 전 게임을 종료해야 파일 잠금과 데이터 손상을 피할 수 있습니다.
