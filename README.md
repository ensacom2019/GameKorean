...
![GameKO 실행 방법: 게임 폴더 선택, Unity 옵션 두 개 선택, 자동 번역 시작, 완료까지 대기](docs/images/quick-start.png)

1. **게임 선택:** `폴더 선택`을 누르고 게임 실행 파일이 들어 있는 폴더를 선택합니다.
2. **옵션 2개 체크:** `Unity 정적 패치(테스트)`와 `버전 무시하고 TMP 강제 적용`을 모두 체크합니다.
3. **자동 번역 진행:** `자동 번역 시작`을 누릅니다.
4. **완료까지 대기:** 진행 중에는 게임이나 GameKO를 종료하지 말고, 완료 팝업이 나타날 때까지 기다립니다.

위의 체크 옵션 두 개는 Unity 게임용입니다. GameMaker와 RPG Maker에서는 체크되어 있어도 Unity 전용 기능은 실행되지 않습니다. `버전 무시하고 TMP 강제 적용`은 Unity 버전에 따라 폰트 호환 문제가 생길 수 있으므로, 실행 오류가 발생하면 체크를 끄고 다시 시도하세요.

### 처음 실행하기 전 준비

Python이나 API 키는 필요하지 않습니다.

1. [최신 릴리스](https://github.com/ensacom2019/GameKorean/releases/latest)에서 `GameKO-Windows.zip`을 받습니다.
2. ZIP을 새 폴더에 완전히 풉니다. 압축 파일 안에서 바로 실행하거나 이전 버전에 덮어쓰면 안 됩니다.
3. 번역할 게임을 종료합니다. Steam에서도 실행 중이 아닌지 확인합니다.
4. 압축을 푼 폴더의 `GameKO.exe`를 실행합니다.
5. 번역 방식은 `웹 자동 번역 (키 없음)`, 원문 언어는 `자동`으로 두면 됩니다.
6. 위 이미지의 1~4번 순서대로 진행합니다.
7. 완료 후 게임을 실행해 번역과 한글 폰트를 확인합니다.
